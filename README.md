...
Linux 提供 deb / rpm / AppImage，Windows 提供 NSIS 安装包：推送 `v*` 标签时由 GitHub Actions 自动构建并发布到 [Releases](https://github.com/assassin321/pure-clash/releases)，标签版本必须与 Cargo 包版本一致。deb/rpm 安装到 `/opt/pure-clash` 并创建 `/usr/bin/pure-clash` 软链，AppImage 开箱即用，所有发行包均携带内核、Geo 数据、清单与许可证文件。
...
- [Pure Clash](https://github.com/prime-zt/pure-clash/releases) 使用 Rust 和 Zed GPUI 构建的轻量原生 Mihomo 桌面客户端
